...
def plot_parsed_status_distribution(runs_dir: Path, output_path: Path) -> None:
    rows = []
    for path in sorted(runs_dir.glob("*/predictions.csv")):
...
def plot_prediction_distribution(runs_dir: Path, output_path: Path) -> None:
    rows = []
    for path in sorted(runs_dir.glob("*/predictions.csv")):
...
def plot_per_author_heatmap(path: Path, output_path: Path) -> None:
    if not path.exists():
...
def main() -> int:
    ...
    plot_parsed_status_distribution(runs_dir, plots_dir / "parsed_status_distribution.png")
    plot_prediction_distribution(runs_dir, plots_dir / "prediction_distribution_by_run.png")
    plot_per_author_heatmap(tables_dir / "phase4_per_author_f1.csv", plots_dir / "per_author_f1_heatmap.png")
    return 0
